...
SRC = BASE / "data_raw" / "eurostat" / "tour_occ_nim.json"
OUT = BASE / "data_processed" / "monthly_proxy_blocks" / "proxy_accommodation_monthly_2017_2025.csv"
...
def main() -> None:
    ...
    for col, preferred in [
        ("unit", ["NR", "NIGHTS", "THS_NIGHT", "NIGHT"]),
        ("c_resid", ["TOTAL"]),
        ("nace_r2", ["I551-I553", "I55"]),
    ]:
    ...
    agg = candidate.groupby(["year", "month"], as_index=False).agg(
        accommodation_nights_spent=("value", "sum")
    )

    spine = build_monthly_spine("2017-01", "2025-12")
    out = spine.merge(agg, on=["year", "month"], how="left")

    out["source_dataset"] = "tour_occ_nim"
    out["source_geo"] = "MT"
    out["source_scope"] = "Eurostat monthly accommodation proxy (nights spent, Malta)"
    ...
    print("saved:", OUT)
    print("rows:", len(out))
    print("non_null accommodation_nights_spent:", out["accommodation_nights_spent"].notna().sum())
    print(out.loc[out["accommodation_nights_spent"].notna(), ["year", "month", "accommodation_nights_spent"]].head(20).to_string(index=False))
